...
1. Install the required version of Python via `pyenv`, and _remember to enable it as a shared library_:
...
4. Create `~/.numpy-site.cfg` pointing to _your OpenBLAS installation directory._
   ...
   (Brew installs OpenBLAS in `/usr/local/opt/openblas/`. Building it from source as above, you installed it in `/opt/OpenBLAS/`.)
