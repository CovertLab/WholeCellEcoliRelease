...
class RnaDegradation(wholecell.processes.process.Process):
	...
	def calculateRequest(self):
		...
		# Full capacity of endonucleases to hydrolyze fragments generated by RNAs degraded (2 molecucles of H2O required per endo-cleavage)
		# Modeling assumption: all full RNAs degraded at a given time step could potentially be sub-fragmented by 
		# other endoRNases
		nFragmentsTotalToCleavage = KcatEndoRNaseFragmentMax * self.endoRnases.total().sum()
		metaboliteUsage[4] += nFragmentsTotalToCleavage
	...
	def evolveState(self):
		...
		# Endocleavages
		nFragmentsTotalToCleavage = KcatEndoRNaseFragmentMax * self.endoRnases.counts().sum()
		H2Oconsumption = 2 * self.rnas.counts().sum() + nFragmentsTotalToCleavage
		Hproduction = self.rnas.counts().sum()
		# Modeling assumption: endolytic fragmentation do no produce PIs
