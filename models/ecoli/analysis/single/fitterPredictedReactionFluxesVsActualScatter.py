...
def main(simOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	scatterArrayActual, scatterArrayPredicted, labels = [], [], []
	for fluxName, predictedFlux in fitterPredictedFluxesDict.iteritems():
		reactionIdx = list(reactionIDs).index(fluxName)
		for timeStepFluxValue in np.random.choice(reactionFluxes[BURN_IN_PERIOD:,reactionIdx], size=NUM_TO_SAMPLE_PER_FLUX, replace=False):
			scatterArrayActual.append(timeStepFluxValue)
	...
	plt.suptitle("Fitter Predicted Fluxes, {} points sampled per prediction, {} step burn-i.".format(NUM_TO_SAMPLE_PER_FLUX, BURN_IN_PERIOD))
