...
def main(simOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	# plt.loglog(targetAve[mixedReactions], actualAve[mixedReactions], "go")
	...
	plt.loglog([1e-12, 1], [1e-13, 0.1], '--r')
	plt.xlabel("Target")
	plt.ylabel("Actual")
	# plt.legend(["kcat and kM", "kcat only"], fontsize = 8, loc = "upper left")

	plt.subplot(4, 1, 2)
	plt.bar(xrange(len(aveError)), sorted(np.log10(aveError), reverse = True))
	plt.xlabel("Reaction")
	plt.ylabel("log(error)")

	plt.subplot(4, 1, 3)
	plt.semilogy(time[BURN_IN_STEPS:], actualFluxes[BURN_IN_STEPS:, :])
	plt.xlabel("Time")
	plt.ylabel("Flux")
	plt.title("Actual Fluxes")

	plt.subplot(4, 1, 4)
	plt.semilogy(time[BURN_IN_STEPS:], targetFluxes[BURN_IN_STEPS:, :])
	plt.xlabel("Time")
	plt.ylabel("Flux")
	plt.title("Target Fluxes")

	# print reactions with flux an order of magnitude above target
	# sortedReactions = [constrainedReactions[x] for x in np.argsort(aveError)[::-1]]
	# for i, error in enumerate(sorted(aveError, reverse = True)):
	# 	if error > 10:
	# 		print "%s: %f" % (sortedReactions[i], error)
