...
def main(simOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	homeostaticTargetMolecules = np.array(fbaData.readAttribute("homeostaticTargetMolecules"))
	...
	grid = gridspec.GridSpec(1,3,wspace=0.0,hspace=0.0,width_ratios=[0.5,1,0.05])

	ax_dendro = fig.add_subplot(grid[0])

	normalized = (
		homeostaticObjectiveComponents
		/ (np.mean(np.abs(homeostaticObjectiveComponents), 0) + 2 * np.std(np.abs(homeostaticObjectiveComponents), 0))
		).transpose()

	linkage = sch.linkage(homeostaticObjectiveComponents.T, metric = "correlation")
	...
	ax_mat.set_yticklabels(homeostaticTargetMolecules[np.array(index)], size = 4)
	...
	plt.title("Homeostatic objective component values", size = 8)
