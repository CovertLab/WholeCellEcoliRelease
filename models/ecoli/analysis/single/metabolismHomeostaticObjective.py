#!/usr/bin/env python
"""
Plot objective homeostatic moelcule components over time
...
def main(simOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	idToColor = {}
	for moleculeID, color in itertools.izip(homeostaticTargetMolecules, itertools.cycle(COLORS_LARGE)):
		idToColor[moleculeID] = color
	...
	with warnings.catch_warnings() as w:
		...
		maxima = np.nanmax(homeostaticObjectiveComponents[BURN_IN_PERIOD:], axis=0)
	maxima = np.delete(maxima, np.where(np.isnan(maxima)))
	threshold_value = sorted(maxima)[-(MAX_NUM_DEVIANTS_TO_SHOW+1)]

	alwaysZero = set()
	for idx, (moleculeID, objectiveValues) in enumerate(zip(homeostaticTargetMolecules, homeostaticObjectiveComponents.T)):
		...
		# Unadjusted
		plt.subplot(2,2,1)
		plt.plot(time / 60., objectiveValues, label=moleculeID, color=idToColor[moleculeID])

		# Log scale
		plt.subplot(2,2,3)
		plt.plot(time / 60., logObjectiveValues, label=moleculeID, color=idToColor[moleculeID])
		...
		if not objectiveValues[BURN_IN_PERIOD:].any():
			alwaysZero.add(moleculeID)

	plt.suptitle("Metabolism Kinetic Objective Components: {} ({:.1f}%) equal zero after a {} step burn-in.".format(len(alwaysZero), 100*len(alwaysZero)/len(homeostaticTargetMolecules), BURN_IN_PERIOD))
	plt.subplot(2,2,1)
