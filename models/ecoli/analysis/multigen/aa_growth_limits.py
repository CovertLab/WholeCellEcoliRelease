...
class Plot(multigenAnalysisPlot.MultigenAnalysisPlot):
	def do_plot(self, seedOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		aa_count_diff = read_stacked_columns(cell_paths, 'GrowthLimits', 'aaCountDiff', remove_first=True)
		...
		for i, (supply, aa_conc, synthesis, imported, use, diff, target, charged) in enumerate(zip(
			...
			ax.plot(times, diff, label='AA counts diff', alpha=0.5)
			ax.plot(times, target, label='AA targets', alpha=0.5)
			ax.plot(times, charged, label='AA trna charged', alpha=0.5)
