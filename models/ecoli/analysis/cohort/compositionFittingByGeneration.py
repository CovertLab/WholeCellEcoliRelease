...
def main(variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	for gen in range(n_gen):
		...
		gs = gridspec.GridSpecFromSubplotSpec(4, 3, subplot_spec=outer_grid[gen])
		...
		maxTime = getMaxTime(generationCells)

		## Load values ##
		time = getSingleValue(generationCells, tableName = "Main", colName = "time", maxTime = maxTime)
		growthRate = getSingleValue(generationCells, tableName = "Mass", colName = "instantaniousGrowthRate", maxTime = maxTime)
		rnaMass = getSingleValue(generationCells, tableName = "Mass", colName = "rnaMass", maxTime = maxTime)
		proteinMass = getSingleValue(generationCells, tableName = "Mass", colName = "proteinMass", maxTime = maxTime)
		dryMass = getSingleValue(generationCells, tableName = "Mass", colName = "dryMass", maxTime = maxTime)
