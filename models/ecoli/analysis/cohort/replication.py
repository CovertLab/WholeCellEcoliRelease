...
def main(variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile = None, metadata = None):
	...
	for simDir in allDir:
		try:
			...
			# downsample = 10
			# axesList[0].plot(time[::downsample], sequenceLength[::downsample,::downsample], marker=',', markersize=1, linewidth=0)
			...
			axesList[3].plot([0, time.max()],[criticalInitiationMass, criticalInitiationMass])
			mass.close()
