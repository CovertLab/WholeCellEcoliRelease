...
def sum_monomers_reference_implementation(sequenceMonomers,
        activeSequencesIndexes, Index currentStep):
    """
...
def sum_monomers(sequenceMonomers, np.int64_t[:] activeSequencesIndexes,
    ...
    """
    return _sum_monomers(sequenceMonomers.view(dtype=np.uint8),
        activeSequencesIndexes, currentStep)
...
def _sum_monomers(np.uint8_t[:, :, ::1] sequenceMonomers not None,
        np.int64_t[::1] activeSequencesIndexes not None, Index currentStep):
